...
public:
  ...
  PrimarySpi(const Options &options = Options()) {
    ...
    spi_mmap_ =
        SystemMmap(fd_, 4096, bcm_host_get_peripheral_address() + SPI_BASE);
...
public:
  ...
  AuxSpi(const Options &options = Options()) {
    ...
    spi_mmap_ =
        SystemMmap(fd_, 4096, bcm_host_get_peripheral_address() + AUX_BASE);
...
struct DeviceImuConfiguration {
  ...
  bool operator==(const DeviceImuConfiguration &rhs) const {
    return yaw_deg == rhs.yaw_deg && pitch_deg == rhs.pitch_deg &&
           roll_deg == rhs.roll_deg && rate_hz == rhs.rate_hz;
  }
...
struct DeviceCanConfiguration {
  ...
  bool operator==(const DeviceCanConfiguration &rhs) const {
    return slow_bitrate == rhs.slow_bitrate &&
           fast_bitrate == rhs.fast_bitrate && fdcan_frame == rhs.fdcan_frame &&
           ...
           bus_monitor == rhs.bus_monitor && std_rate == rhs.std_rate &&
           fd_rate == rhs.fd_rate;
  }
...
public:
  ...
  void ConfigureAux() {
    ...
    if (desired_imu != original_imu_configuration) {
      primary_spi_.Write(0, 36, reinterpret_cast<const char *>(&desired_imu),
                         sizeof(desired_imu));

      // Give it some time to work.
      ::usleep(1000);
      DeviceImuConfiguration config_verify;
      primary_spi_.Read(0, 35, reinterpret_cast<char *>(&config_verify),
    ...
    primary_spi_.Read(0, 49, reinterpret_cast<char *>(&original_id),
                      sizeof(original_id));
    if (original_id != config_.rf_id) {
      primary_spi_.Write(0, 50, reinterpret_cast<const char *>(&config_.rf_id),
                         sizeof(uint32_t));
      // Changing the ID takes at least a few milliseconds.
      ::usleep(10000);
      uint32_t id_verify = 0;
      primary_spi_.Read(0, 49, reinterpret_cast<char *>(&id_verify),
                        sizeof(id_verify));
      ThrowIf(config_.rf_id != id_verify, [&]() {
        return Format("RF Id not set properly (%08x != %08x)", config_.rf_id,
                      id_verify);
      });
    }
  }
  ...
  void UpdateCanConfig(Spi *spi, int cs, int canbus,
    ...
    spi->Read(cs, canbus ? 8 : 7, reinterpret_cast<char *>(&original_config),
    ...
    spi->Write(cs, canbus ? 10 : 9, reinterpret_cast<const char *>(&out),
               sizeof(out));

    // Give it some time to work.
    ::usleep(100);
    DeviceCanConfiguration verify;
    spi->Read(cs, canbus ? 8 : 7, reinterpret_cast<char *>(&verify),
              sizeof(verify));
    ThrowIf(out != verify,
  ...
  template <typename Spi> void TestCan(Spi *spi, int cs, const char *name) {
    ...
    if (version != 2 && version != 3) {
      throw std::runtime_error(
          Format("Processor '%s' has incorrect CAN SPI version %d != [2,3]",
                 name, version));
    }
  }
  ...
  void VerifyVersions() {
    ...
    if (config_.enable_aux) {
      ...
      if (attitude_version != kAttitudeVersion) {
        throw std::runtime_error(Format("Incorrect attitude version %d != %d",
  ...
  bool GetAttitude(Attitude *output, bool wait, bool detail) {
    ...
    do {
      primary_spi_.Read(0, 34, reinterpret_cast<char *>(&device_attitude_),
  ...
  template <typename Spi>
  void SendCanPacketSpi(Spi &spi, int cs, int cpu_bus,
  ...
  void SendRf(const Span<RfSlot> &slots) {
    ...
    for (size_t i = 0; i < slots.size(); i++) {
      ...
      primary_spi_.Write(0, 51, reinterpret_cast<const char *>(&buf[0]),
  ...
  void ReadRf(const Input &input, Output *output) {
    ...
    primary_spi_.Read(0, 52, reinterpret_cast<char *>(&rf_status),
                      sizeof(rf_status));
    ...
    for (int i = 0; i < 15; i++) {
      ...
      primary_spi_.Read(0, 64 + i, reinterpret_cast<char *>(&slot_data),
                        sizeof(slot_data));
  ...
  int ReadCanFrames(Spi &spi, int cs, int bus_start,
    ...
    while (true) {
      ...
      spi.Read(cs, 2, reinterpret_cast<char *>(&queue_sizes[0]),
               sizeof(queue_sizes));
