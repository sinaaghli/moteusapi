...
int seriallib::serialport_init(const char* serialport, int baud) {
...
//
int seriallib::serialport_close(int fd) { return close(fd); }

//
int seriallib::serialport_writebyte(int fd, uint8_t b) {
  int n = write(fd, &b, 1);
  if (n != 1) return -1;
  return 0;
}
...
//
int seriallib::serialport_write(int fd, const char* str) {
  int len = strlen(str);
...
//
int seriallib::serialport_read_until(int fd, char* buf, char until, int buf_max,
                                     int timeout) {
...
//
int seriallib::serialport_flush(int fd) {
  sleep(2);  // required to make flush work, for some reason
  return tcflush(fd, TCIOFLUSH);
}

// Addition by Sebastian Giles
int seriallib::serialport_readbyte(int fd) {
  char b[1];  // read expects an array, so we give it a 1-byte array
...
// Addition by Sebastian Giles
int seriallib::serialport_readbytes(int fd, char* buf, int len) {
  char b[1];  // read expects an array, so we give it a 1-byte array
  int i = 0;
  do {
    int n = read(fd, b, 1);  // read a char at a time
...
int seriallib::serialport_writebytes(int fd, const char* str, int len) {
  int n = write(fd, str, len);
  if (n != len) {
